...
async function seed() {
  ...
  await db.insert(schema.users).values([
    // Students
    { id: U_ARJUN,   email: 'arjun.sharma@college.edu',   fullName: 'Arjun Sharma',    roles: ['student'] },
    ...
    { id: U_SNEHA,   email: 'sneha.menon@college.edu',    fullName: 'Sneha Menon',     roles: ['student'] },
    { id: U_ROHAN,   email: 'rohan.gupta@college.edu',    fullName: 'Rohan Gupta',     roles: ['student'] },
    // Recruiters
    { id: U_RECRUITER_BYTE, email: 'hr@byteforge.io',    fullName: 'ByteForge HR',    roles: ['recruiter'] },
    { id: U_RECRUITER_NEXA, email: 'talent@nexahire.com', fullName: 'NexaHire Talent', roles: ['recruiter'] },
    // Club admins (also students in real life)
    { id: U_ADMIN_ACM,    email: 'acm-lead@college.edu',    fullName: 'Dev Kumar',    roles: ['student', 'club_admin'] },
    { id: U_ADMIN_DESIGN, email: 'design-lead@college.edu', fullName: 'Ananya Iyer',  roles: ['student', 'club_admin'] },
    // Core admin
    { id: U_CORE, email: 'admin@hire-me.college', fullName: 'Platform Admin', roles: ['core_admin'] },
  ]).onConflictDoNothing();
  ...
  await db.insert(schema.clubAdmins).values([
  ...
  await db.insert(schema.studentProfiles).values([
    ...
    },
    {
      ...
      otherLinks: { portfolio: 'https://priyanair.design', behance: 'https://behance.net/priyanair' },
      dk24Status: 'verified',
  ...
  await db.insert(schema.contactDetails).values([
  ...
  await db.insert(schema.projects).values([
    {
      ...
      description: 'Campus job-board with DK24 student verification, club management, and recruiter portal.',
      role: 'Full-Stack Lead',
      contributions: 'Designed the database schema, built REST API with Hono, and implemented the React frontend.',
    ...
    },
    {
      ...
      description: 'Lightweight kanban board for small engineering teams with real-time updates via WebSockets.',
    ...
    },
    {
      ...
      description: 'Full UX redesign of a mental wellness mobile app. Led usability testing with 30 participants.',
      role: 'Lead Designer',
      contributions: 'End-to-end design: user research, wireframes, high-fidelity prototypes, and handoff specs.',
    ...
    },
    {
      ...
      description: 'Built a PID-controlled line-following robot for the inter-college robotics fest.',
      role: 'Hardware & Firmware Lead',
      contributions: 'PCB design, motor control firmware in C++, and computer vision integration with OpenCV.',
    ...
    },
    {
      studentId: U_ROHAN,
      title: 'SentimentLens',
      description: 'Fine-tuned BERT model for aspect-based sentiment analysis on product reviews.',
      role: 'ML Engineer',
      contributions: 'Data pipeline, model fine-tuning, evaluation on SemEval dataset, and a Gradio demo.',
  ...
  await db.insert(schema.experience).values([
    {
      ...
      endDate: '2024-07-31',
      contributions: 'Built a PDF parsing microservice using Node.js and pdfjs. Reduced processing time by 40%.',
    },
    {
      studentId: U_PRIYA,
      ...
      endDate: '2024-08-31',
      contributions: 'Revamped the brand identity for 3 client projects. Delivered Figma-to-HTML asset exports.',
    },
    {
      studentId: U_KIRAN,
      ...
      endDate: null,
      contributions: 'Building a high-throughput event pipeline in Go using Kafka and Redis Streams.',
    },
    {
      studentId: U_SNEHA,
  ...
  await db.insert(schema.achievements).values([
    ...
    },
    {
      ...
      description: 'Co-authored a paper on aspect-level sentiment analysis accepted at EMNLP 2025 student workshop.',
      date: '2025-07-01',
    },
  ]).onConflictDoNothing();
  ...
  await db.insert(schema.postings).values([
    {
      ...
      description: 'Join ByteForge to build our core developer tooling platform. You will own features end-to-end, from API design to frontend delivery. We use TypeScript, React, and Node.js on a Cloudflare Workers edge stack.',
    ...
    },
    {
      ...
      description: 'We are looking for a design intern who can own user research, wireframes, and high-fidelity prototypes for ByteForge\'s flagship product. Strong Figma skills required.',
    ...
    },
    {
      ...
      description: 'NexaHire is scaling its matching engine. We need a backend engineer with strong experience in distributed systems, Go or Rust, and event-driven architectures.',
    ...
    },
    {
      ...
      description: 'Short-term contract to fine-tune and evaluate transformer models for resume-to-job-description matching. Must be comfortable with HuggingFace and model evaluation metrics.',
  ...
  await db.insert(schema.applications).values([
    ...
    { postingId: P_FULLSTACK, studentId: U_ARJUN, status: 'shortlisted',
      appliedAt: new Date('2025-08-05T10:30:00Z'), updatedAt: new Date('2025-08-08T09:00:00Z') },
    // Priya → Design Intern (under review)
    { postingId: P_DESIGN_IN, studentId: U_PRIYA, status: 'under_review',
      appliedAt: new Date('2025-08-06T14:00:00Z') },
    // Arjun → Backend (just applied, stretch role)
    { postingId: P_BACKEND, studentId: U_ARJUN, status: 'applied',
      appliedAt: new Date('2025-08-09T18:00:00Z') },
    // Kiran → Backend (under review)
    { postingId: P_BACKEND, studentId: U_KIRAN, status: 'under_review',
      appliedAt: new Date('2025-08-07T11:00:00Z') },
    // Rohan → ML Contract (applied)
    { postingId: P_CONTRACT, studentId: U_ROHAN, status: 'applied',
      appliedAt: new Date('2025-08-08T08:00:00Z') },
    // Sneha → Full-Stack (rejected — not matching)
    { postingId: P_FULLSTACK, studentId: U_SNEHA, status: 'rejected',
      appliedAt: new Date('2025-08-04T09:00:00Z'), updatedAt: new Date('2025-08-07T15:00:00Z') },
  ]).onConflictDoNothing();
  ...
  await db.insert(schema.clubMemberships).values([
    ...
    {
      id: CM_ARJUN_ACM,
      clubId: C_ACM, userId: U_ARJUN,
      role: 'member', fullName: 'Arjun Sharma',
      usn: '1CS21CS010', email: 'arjun.sharma@college.edu',
    },
    {
      id: CM_KIRAN_ACM,
      clubId: C_ACM, userId: U_KIRAN,
      role: 'member', fullName: 'Kiran Rao',
      usn: '1CS22CS045', email: 'kiran.rao@college.edu',
    },
    // Admin of ACM — also a member
    {
      clubId: C_ACM, userId: U_ADMIN_ACM,
      role: 'admin', fullName: 'Dev Kumar',
      usn: '1CS21CS001', email: 'acm-lead@college.edu',
    },
    // Design Collective
    {
      id: CM_PRIYA_DESIGN,
      clubId: C_DESIGN, userId: U_PRIYA,
      role: 'member', fullName: 'Priya Nair',
      usn: '1CS21CS022', email: 'priya.nair@college.edu',
    },
    {
      clubId: C_DESIGN, userId: U_ADMIN_DESIGN,
      role: 'admin', fullName: 'Ananya Iyer',
      usn: '1CS21CS003', email: 'design-lead@college.edu',
    },
    // Robotics Club
    {
      id: CM_SNEHA_ROBOT,
      clubId: C_ROBOT, userId: U_SNEHA,
      role: 'member', fullName: 'Sneha Menon',
      usn: '1EE22EE015', email: 'sneha.menon@college.edu',
    },
    // Rohan's record has a USN mismatch — no userId linked yet
    {
      clubId: C_ACM, userId: null,
      role: 'member', fullName: 'Rohan Gupta',
  ...
  await db.insert(schema.notifications).values([
    ...
    // Rohan: verification failed
    {
      ...
      payload: {
        result: 'rejected',
        reason: 'USN mismatch — submitted 1CS21CS088, club record has 1CS21CS099. Please resubmit with the correct USN.',
      },
